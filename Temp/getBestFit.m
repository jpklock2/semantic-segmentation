function [gmBest] = getBestFit(X, plots)
...
bicMatrix = reshape(bic,nK*nSigma*nSC, 1);
aicMatrix = reshape(aic,nK*nSigma*nSC, 1);
grades = 1/20:1/20:1;
gradesClust = zeros(20, 1);
[~, idxBic] = sort(bicMatrix);
[~, idxAic] = sort(aicMatrix);
for i = 1:length(grades)
    gradesClust(i) = grades(idxBic(i)) + grades(idxAic(i));
end
bestFit = find(gradesClust == min(gradesClust), 1);
bestK = ceil(bestFit/(nSigma*nSC));
switch mod(bestFit, nSigma*nSC)
    case 1
        bestSig = 1;
        bestSc = 1;
    case 2
...
gmBest = gm{bestK,bestSig,bestSc};
