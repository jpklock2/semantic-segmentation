...
% [~, idx2] = sort(U, 2);
% idx2 = idx2(:, end);
% CH(iter) = evalclusters(X, idx2, 'CalinskiHarabasz').CriterionValues; % maior, melhor
% DB(iter) = evalclusters(X, idx2, 'DaviesBouldin').CriterionValues; % menor, melhor
% ST(iter) = evalclusters(X, idx2, 'silhouette').CriterionValues; % maior, melhor
