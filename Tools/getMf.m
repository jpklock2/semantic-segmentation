...
function [outFis] = getMf(X, classes, centroids, U)

numInp = size(X, 2);
numOutp = 1;
cluster_n = size(classes, 2);
U_temp = U';
...
for i = 1:numInp
    ...
    for j = 1:1:cluster_n
        ...
        mf(j).Parameters = computemfparams ('gaussmf', X(:,i), U_temp(j,:)', centroids(j,i));
    end   
...
for i=1:1:numOutp
    ...
    for j = 1:1:cluster_n
        ...
        mfS(j).Parameters = computemfparams ('linear', [X classes(:,i)]);
    end
