...
# para log_coords1
##################
log_coords <- data.frame(readMat(paste(pasta, '/', log , '.mat', sep="")))
colnames(log_coords) <- c('lat_ref', 'lon_ref', 'lat_est', 'lon_est', 'dist',
                        'case','frame')

log_coords <- head(log_coords,-2)
# Coleta de erros
ERRO_caso_1 <- data.frame(subset(log_coords$dist, log_coords$case == 1))
colnames(ERRO_caso_1)<-c("ERRO_caso_1")

ERRO_caso_2 <- data.frame(subset(log_coords$dist, log_coords$case == 2))
colnames(ERRO_caso_2)<-c("ERRO_caso_2")

ERRO_caso_3 <- data.frame(subset(log_coords$dist, log_coords$case == 3))
colnames(ERRO_caso_3)<-c("ERRO_caso_3")

ERRO_caso_4 <- data.frame(subset(log_coords$dist, log_coords$case == 4))
colnames(ERRO_caso_4)<-c("ERRO_caso_4")

ERRO_caso_5 <- data.frame(subset(log_coords$dist, log_coords$case == 5))
colnames(ERRO_caso_5)<-c("ERRO_caso_5")

ERRO_caso_6 <- data.frame(subset(log_coords$dist, log_coords$case == 6))
colnames(ERRO_caso_6)<-c("ERRO_caso_6")

ERRO_caso_7 <- data.frame(subset(log_coords$dist, log_coords$case == 7))
colnames(ERRO_caso_7)<-c("ERRO_caso_7")

x <- c(1:nrow(ERRO_caso_1))
...
xx=(rep(1:nrow(ERRO_caso_1),7))
ERROR_22 <- data.frame(ERROR_2,xx)
ERROR_22a<-(ERROR_22[1:(nrow(ERRO_caso_1)*4),])
...
ERROR_22b<-(ERROR_22[(nrow(ERRO_caso_1)*3):length(ERROR_22[,1]),])
